...
const DisplayTodos = ({
    ...
    return (
        ...
        {data.length ? (
            <div>
                <div
                className="max-md:container border text-white max-w-[700px] mt-10 max-sm:mt-2 m-auto rounded-3xl bg-gradient-to-r from purple-500 to-purple-700 p-10 max-sm:p-5"
                >
                    ...
                    <div className="flex flex-col w-[60%] max-sm:w-[100%] mt-7 max-sm:mt-5">
                        <div className="flex justify-between items-center">
                            <p className="max-sm:text-xs">
                                Progress
                                <span className={`text-sm max-sm:text-xs ${handleTasksStatus() === "No tasks completed" 
                                ? "text-red-700" : handleTasksStatus === "Less than half tasks completed" 
                                ? "text-red-700" : "text-green-500"} font-semibold`}>({handleTasksStatus()})</span>                                       
                            </p>
                ...
                <div 
                className="max-md:container max-w-[700px] m-auto mt-7 max-sm:mt-5 mb-7 max-sm:mb-5 relative"
                > 
                <input 
                type="text"
                ...
                <div className="max-md:container max-w-[700px] m-auto flex flex-col gap-4 max-sm:gap-3 pb-5">
                    {searchResults.map((val, index) => (
                        ...
                        setData={setData}
                        setEdit={setEdit}
                        setDeleteNotification={setDeleteNotificationTitle}
                        setDeleteNotificaiton={setDeleteNotificaiton}
            ...
            <h1 className="w-full text-center text-2xl max-md:text-2xl max-sm:text-xl text-white font-bold absolute bottom-[50%] -translate-x-[50%">
